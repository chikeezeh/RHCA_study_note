...
Plugin is an extension to the Ansible code that is written in Python. Ansible core comes with some standard plugins, and additional plugins can be installed via content collections. Use this command `ansible-doc -t plugin-type -l` to get a list of plugins in a plugin-type. For example, the list of `lookup` plugins installed currently in my control node are below:
